...
class AxisWiseDropout(nn.Module):
    def __init__(self, p: int, dim: Union[int, str]):
        """
        Args:
            p: probability of zeroing out
            dim: dimension across which to share the mask (e.g. -1 for the last dimension)
                'per_row' is equivalent to -3
                'per_column' is equivalent to -2
        """
        super().__init__()
        if isinstance(dim, str):
            if dim == 'per_row':
                ...
                raise ValueError(f"Incorrect orientation {dim}")
        self.p = p
        self.dim = dim
